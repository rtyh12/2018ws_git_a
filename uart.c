...
// USART-Init
void uartInit() {
	UCSR0B |= (1 << TXEN0) | (1 << RXEN0);	// UART Senden und empfangen
	// einschalten
	UBRR0 = UBRR_VAL;	// Baudrate einstellen, die im Header-File
	// definiert wurde
}
...
int uart_putc(unsigned char c) {
	...
	// warte, bis UDR bereit ist,
	// d.h. letztes Zeichen den
	// Sendepuffer verlassen hat
...
void uart_puts_pgm(const char *PROGMEM str) {
	while (1) {
		// so lange c != '\0' also ungleich dem
		// "String-Endezeichen" 
